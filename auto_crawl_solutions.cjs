...
const items = [];
[...Array(27).keys()].forEach(i => {
    let num = (i + 1).toString().padStart(2, '0');
    console.log(`Fetching Solution_${num}...`);
    let prod = parsePage(`https://cs.new.myibc.net/Solution_${num}`);
    if (prod) items.push(prod);
});

fs.writeFileSync('extracted_solutions.json', JSON.stringify(items, null, 2));
console.log("Done.");
